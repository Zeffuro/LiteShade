...
public static class ConfigBackup
{
    ...
    private const string Name = "LiteShade";
    ...
    public static void DoConfigBackup(IDalamudPluginInterface pluginInterface)
    {
        try
        {
            ...
            var launcherDirectory = pluginInterface.ConfigFile.Directory?.Parent ?? pluginInterface.ConfigDirectory.Parent?.Parent;
            ...
            if (latestFile.Exists)
            {
                try
                {
                    needsBackup = ZipJsonHash(latestFile.FullName) != ConfigFilesJsonHash(configFiles, pluginInterface);
                }
                catch (InvalidDataException exception)
                {
                    IPluginLog.Get().Warning(exception, "Replacing an unreadable configuration backup.");
                }
            }
            if (!needsBackup)
            {
            ...
            if (latestFile.Exists)
            {
                ...
                var archiveName = $"{Name}.{timestamp:yyyyMMddHHmmssfff}.{Guid.NewGuid():N}.zip";
        ...
        {
            IPluginLog.Get().Warning(exception, "Configuration backup skipped.");
        }
    }
